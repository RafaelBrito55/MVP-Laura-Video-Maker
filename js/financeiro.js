...
// ===== Categorias (select por tipo: receita x despesa) =====
function finGetCategoriasFromDatalist(listId) {
  const dl = document.getElementById(listId);
  if (!dl) return [];
  const out = [];
  dl.querySelectorAll("option").forEach((o) => {
    const v = (o.value || o.getAttribute("value") || (o.textContent || "").trim());
    if (v) out.push(v);
  });
  return out;
}

function finFillSelectOptions(sel, values, keepValue) {
  if (!sel) return;
  const uniq = [];
  const seen = new Set();
  const push = (v) => {
  ...
  // Mantém categoria já existente (caso venha de edição / dado antigo)
  if (keepValue) push(keepValue);
  (values || []).forEach(push);
  ...
  if (keepValue && uniq.includes(String(keepValue).trim())) {
    sel.value = String(keepValue).trim();
  } else if (uniq.length) {
    sel.value = uniq[0];
  }
}

function finSyncCategoriasSelect() {
  ...
  const current = String(elFin.lanCategoria.value || "").trim();
  const values = finGetCategoriasFromDatalist(listId);

  // Regra:
  // - Se a categoria atual existe na lista do tipo escolhido, mantém.
  // - Se estiver editando e a categoria atual não existe na lista (dado antigo), mantém como opção extra.
  // - Caso contrário (troca de tipo em lançamento novo), reseta para a 1ª opção do tipo.
  let keepValue = "";
  if (current && values.includes(current)) keepValue = current;
  else if (editingLancamentoId && current) keepValue = current;

  finFillSelectOptions(elFin.lanCategoria, values, keepValue);
}
...
finOn(elFin.lanTipo, "change", finSyncCategoriasSelect);
