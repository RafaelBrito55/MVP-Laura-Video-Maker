...
(function () {
  ...
  // Helpers de data (dd/mm/aa)
  function pad2(n){ return String(n).padStart(2, "0"); }

  function brToISOFlex(input){
    // Aceita:
    // - dd/mm/aa  ou dd/mm/aaaa (também com hífen)
  ...
  function monthKeyToInput(monthKey){
    ...
    if(!m) return "";
    return `01/${m[1]}/${m[2].slice(-2)}`;
  }

  function maskDateDDMMYY(inputEl){
    if(!inputEl) return;
  ...
  async function projetosRefresh() {
    ...
    if (!range) {
      projetosSetMsg("Mês inválido. Use dd/mm/aa (ex: 15/01/26) ou MM/AAAA (ex: 01/2026).");
      return;
    }

    // Mantém o input sempre em dd/mm/aa (primeiro dia do mês)
...
const { startISO, endISOExclusive } = range;
  ...
  async function projetosSave() {
    ...
    try {
      ...
      if (!dataISO) throw new Error("Data inválida (dd/mm/aaaa).");
      ...
      if (mk && elProj.monthInput && elProj.monthInput.value !== mk) {
        elProj.monthInput.value = mk;
      }
  ...
  // Ajustes de inputs (datas em dd/mm/aa)
